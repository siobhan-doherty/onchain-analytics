...
import duckdb
from dagster import (
    job, op, ScheduleDefinition, Definitions, sensor, 
...
def check_freshness():
    ...
    if hours_since > 168:   # 7 days
        raise Exception(
            f"Data is stale: last update was {hours_since:.1f} hours ago"
        )
...
def data_freshness_sensor(context: SensorEvaluationContext):
    ...
    hours_since = (datetime.now() - last_update).total_seconds() / 3600
    if hours_since > 168:
        context.log.warning(f"Data is stale ({hours_since:.1f} hours). Consider running the pipeline.")
